...
Page({
  ...
  onLoad() { this.setData({ filteredUsers: this.data.users }); },
  onSearch(e) {
    const kw = e.detail.value.trim().toLowerCase();
    const list = kw ? this.data.users.filter(u => u.name.toLowerCase().includes(kw)) : this.data.users;
    this.setData({ keyword: kw, filteredUsers: list });
  },
